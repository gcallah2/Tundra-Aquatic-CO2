...
# OUtlier removed
data_tidy2 <- data %>% mutate(Habitat = tolower(Habitat)) %>% # Change all Habitat names to lowercase
  rename(c_flux = Fmmol_mmol_m2_day, # Simplify key variable names
         air_temp = AirTemp_C) %>%
  filter(air_temp >-100) %>% filter(c_flux >-100) %>% # filter out missing (-9999) values for air temp and C flux
  filter(Habitat != "tk-pond") %>% # remove tk-pond observations due to few (7) observations 
...
# Effect of air temperature on CO2 flux
...
# Linear model (y~x) 
lm_overall <- lm(c_flux ~ air_temp, data = data_tidy2)
summary(lm_overall)
``` 
